  ...
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
                            ...
                            std::vector<Vector<double>> &computed_quantities) const
      {
        ...
        // If requested, convert the shear stress tensor to spherical coordinates
            ...
            for (unsigned int q=0; q<n_quadrature_points; ++q)
              {
                ...
                for (unsigned int d=0; d<dim; ++d)
                  for (unsigned int e=0; e<dim; ++e)
                    shear_stress[d][e] = computed_quantities[q][Tensor<2,dim>::component_to_unrolled_index(TableIndices<2>(d,e))];

                shear_stress = aspect::Utilities::Coordinates::cartesian_to_spherical_tensor(shear_stress, input_data.evaluation_points[q]);
