...
namespace aspect
{
  namespace Postprocess
  {
                                                 ...
                                                 OutputHistory                  &output_history) const
    {
          ...
          if (this->get_nonlinear_iteration() == 0)
            {
              ...
              output_history.timestep_numbers.emplace_back(this->get_timestep_number());
            }
          else
            {
              output_history.times_and_pvtu_names.back() = (std::make_pair
                                                            (time_in_years_or_seconds, "solution/"+pvtu_filename));
              // timestep_numbers remains the same
            }
        }
      else
        {
      ...
      // Custom PVD writing to use 'time' and 'step' attributes
      pvd_file << "<?xml version=\"1.0\"?>\n";
      pvd_file << "<!--\n";
      pvd_file << "#This file was generated by ASPECT on " << Utilities::System::get_date() << " at " << Utilities::System::get_time() << "\n";
      pvd_file << "-->\n";
      pvd_file << "<VTKFile type=\"Collection\" version=\"0.1\" ByteOrder=\"LittleEndian\">\n";
      pvd_file << "  <Collection>\n";
      for (size_t i = 0; i < output_history.times_and_pvtu_names.size(); ++i)
        {
          const double time_val = output_history.times_and_pvtu_names[i].first;
          const std::string &filename = output_history.times_and_pvtu_names[i].second;
          const unsigned int step_val = output_history.timestep_numbers[i];
          pvd_file << "    <DataSet timestep=\"" << time_val << "\" step=\"" << step_val << "\" group=\"\" part=\"0\" file=\"" << filename << "\"/>\n";
        }
      pvd_file << "  </Collection>\n";
      pvd_file << "</VTKFile>\n";
